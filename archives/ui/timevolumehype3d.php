...
<html>
    ...
    <body class="fixed-left">
                        ...
                        <div class="row">
                            <div class="col-sm-12 card-box">
                                <div class="row">
                                    <div class="col-sm-12">
                                        <p>
                                            For more information about this tool and settings please read <a href="https://medium.com/@cryptopredicted/this-is-what-cryptocurrencies-look-like-in-3-dimensions-3d-analysis-part-1-2a629701560" target="_blank">this</a>.
                                        </p>
                                    </div>
                                </div>
                            </div>
        ...
        <script>
            function drawGraphs() {
                data = [];
                data = data.concat(draw_TimeVolumeHype_3d(DATA));
                console.log(data);
            ...
            function requestAndDraw() {
                ...
                ///////
                // for optimal caching purposes::
                // convert times to UTC
                // do modulo operations to downsize hours/minutes
                var now = localToUTC(new Date());
                var dtime = $("#_datepicker").val() == "" ? now : localToUTC($("#_datepicker").val());
                if (dtime > now) {
                    dtime = now;
                }
                var interval = $('#_interval').val();
                if (interval <= 60) {
                    dtime.set({ minute: dtime.minutes() - dtime.minutes()%(interval) });
                } else if (interval <= 1440) {
                    dtime.set({ minute: dtime.minutes() - dtime.minutes()%(60) });
                    dtime.set({ hour: dtime.hours() - dtime.hours()%(interval/60) });
                }
                // write-back maxDateTimeExcluded to datepicker in local timezone
                ...
                $.getJSON('https://cryptopredicted.com/PWA/api/', {   
                        'type':         'exchange,socialMentions,newsMentions',
                        'exchange':     'binance',
